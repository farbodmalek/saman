...
const getPlanNo = (name: string) => {
  ...
  MakeResponse.makeServerResponse(CommonService.getPlanNoList(filter.value), true, result => {
    if (result.results) {
...
const setPlanNo = () => {
  const body = {
    loanId: params.value.id,
    planNoId: planId.value
  }
  MakeResponse.makeServerResponse(CommonService.setPlanNo(body), true, result => {
    if (result.result) {
